...
public class UserServiceTest
{
	...
	{
		Assert.assertEquals("Insufficient amount of users initialized for test data source", 3,
		...
		for (User user : userService.getAllUsers())
		{
			...
			{
				Assert.assertTrue("User \"user1\" does not have role STUDENT",
						user.getRoles().contains(UserRole.STUDENT));
			...
			{
				Assert.assertTrue("User \"user2\" does not have role EMPLOYEE",
						user.getRoles().contains(UserRole.EMPLOYEE));
			...
			{
				Assert.fail("Unknown user \"" + user.getUsername()
	...
	public void testDeleteUser()
	{
		...
		Assert.assertEquals("No user has been deleted after calling UserService.deleteUser", 2,
				userService.getAllUsers().size());
		User deletedUser = userService.loadUser("user1");
		Assert.assertNull("Deleted User1 could still be loaded from test data source via UserService.loadUser",
				deletedUser);

		for (User remainingUser : userService.getAllUsers())
		{
			Assert.assertNotEquals(
					"Deleted User1 could still be loaded from test data source via UserService.getAllUsers",
					toBeDeletedUser.getUsername(), remainingUser.getUsername());
		}
	}
	...
	public void testUpdateUser()
	{
		...
		Assert.assertNotNull("User \"user1\" does not have a updateUser defined after being saved",
		...
		Assert.assertNotNull("User \"user1\" does not have a updateDate defined after being saved",
				freshlyLoadedUser.getUpdateDate());
		Assert.assertEquals("User \"user1\" does not have a the correct email attribute stored being saved",
				"changed-email@whatever.wherever", freshlyLoadedUser.getEmail());
	}
	...
	public void testCreateUser()
	{
		...
		toBeCreatedUser.setRoles(Sets.newSet(UserRole.EMPLOYEE, UserRole.STUDENT));
		userService.saveUser(toBeCreatedUser);

		User freshlyCreatedUser = userService.loadUser("newuser");
		Assert.assertNotNull("New user could not be loaded from test data source after being saved",
				freshlyCreatedUser);
		Assert.assertEquals("User \"newuser\" does not have a the correct username attribute stored being saved",
				"newuser", freshlyCreatedUser.getUsername());
		Assert.assertEquals("User \"newuser\" does not have a the correct password attribute stored being saved",
				"passwd", freshlyCreatedUser.getPassword());
		Assert.assertEquals("User \"newuser\" does not have a the correct firstName attribute stored being saved",
				"New", freshlyCreatedUser.getFirstName());
		Assert.assertEquals("User \"newuser\" does not have a the correct lastName attribute stored being saved",
				"User", freshlyCreatedUser.getLastName());
		Assert.assertEquals("User \"newuser\" does not have a the correct email attribute stored being saved",
				"new-email@whatever.wherever", freshlyCreatedUser.getEmail());
		Assert.assertTrue("User \"newuser\" does not have role STUDENT",
				freshlyCreatedUser.getRoles().contains(UserRole.STUDENT));
		Assert.assertTrue("User \"newuser\" does not have role EMPLOYEE",
				freshlyCreatedUser.getRoles().contains(UserRole.EMPLOYEE));
		Assert.assertNotNull("User \"newuser\" does not have a createUser defined after being saved",
		...
		Assert.assertNotNull("User \"newuser\" does not have a createDate defined after being saved",
